...
def getBarsEachTrack(image:np.ndarray, beamMapImg:np.ndarray, staffList:List[Staff], tolerance:int = 20):
    ...
    for trkRange in pageMetadata.getTrackRange():
        ...
        try:
            ...
            barPos = findBarPosition(staffImgBinary[trkRange[0]:trkRange[1]+1,:], tolerance)
...
def readCsvAndEditViolin(csvPath: str):
    ...
    for i in range(n):
        if pd.notna(df.at[i, 'part1']) and df.at[i, 'part1'] == 1:
            ...
            if not (
                i + 1 < n and
                pd.notna(df.at[i+1, 'ins1']) and df.at[i+1, 'ins1'] == df.at[i, 'ins1'] and
                pd.notna(df.at[i+1, 'part1']) and df.at[i+1, 'part1'] == 2
            ):
                df.at[i, 'part1'] = pd.NA
    return df
...
if __name__ == '__main__':
    ...
    _ap = argparse.ArgumentParser(
        description="Stage 2: page image + layout metadata CSV -> MusicXML. "
                    "Same recognition logic as the paper; only the CLI and the cache cleanup are new.")
    _ap.add_argument("--sheet", default="Bee_1_challenge", help="piece name, e.g. Bee_1_challenge")
    _ap.add_argument("--pages", default="1-10",
                     help="page list/range over the ORIGINAL page numbering, e.g. '1-10' or '3,5,9'")
    _ap.add_argument("--data-root", default="orch_dataset",
                     help="root containing {sheet}/csv and {sheet}/imgs (built by scripts/bridge_metadata_to_omr.py)")
    _ap.add_argument("--first-ts", default="4/4", help="time signature carried into the first page")
    _ap.add_argument("--keep-cache", action="store_true",
                     help="keep the per-page .npy/.pkl decode cache (~660 MB per page!)")
    ...
    sheetName = _args.sheet
    dataRoot = _args.data_root
    pageList = _parse_pages(_args.pages)
    prevBarTS = [int(x) for x in _args.first_ts.split("/")]
    lenString = 3
    runWholeModel = False
    fullBarList = []
    fullksListAllTrack = np.array([])
    # dataSeperate = [1, 5, 13, 27, 34, 39, 44, 49, 62, 73, 85, 98, 104, 110, 124, 138, 149,
    #                  162, 177, 192, 201, 211, 223, 227, 236, 246, 253, 258, 269, 276, 283,
    #                    290, 1, 17, 29, 43, 55, 59, 72, 82, 88, 96, 115, 127, 141, 153, 158,
    #                      171, 183, 1, 17, 35, 51, 60, 70, 79, 96, 121, 130, 1, 11, 26, 34, 41,
    #                        55, 71, 85, 92, 105, 122, 134, 148, 154, 161, 179, 194, 211, 226,
    #                          232, 240, 253, 270, 285, 294]

    for number in pageList:
        isInit = False 
        # isInit = dataSeperate[number-1] == 1
        jsonPath = rf"{dataRoot}/{sheetName}/csv/{sheetName}.json"
        csvPath = rf"{dataRoot}/{sheetName}/csv/{sheetName}_{numberToString(number, lenString)}.csv"
        df = readCsvAndEditViolin(csvPath)
        pageMetadata = ScoreMetaData(df, jsonPath, isInit)
        instrumentList = ScoreMetaData.get_instruments()
        imgName = rf"{sheetName}_{number}"
        imgPath = rf"{dataRoot}/{sheetName}/imgs/{sheetName}_{number}/{sheetName}_{number}.png"
        pklPath = rf"{dataRoot}/{sheetName}/imgs/{sheetName}_{number}/{sheetName}_{number}.pkl"
        scorePath = rf"{dataRoot}/{sheetName}/xmls/{sheetName}_{number}.musicxml"
        scoreShiftedPath = rf"{dataRoot}/{sheetName}/xmls/{sheetName}_{number}_shifted.musicxml"
        if not os.path.isdir(rf"{dataRoot}/{sheetName}/xmls"):
            os.mkdir(rf"{dataRoot}/{sheetName}/xmls")
        ...
        print("checkpoint the page number is ", number)
        ...
        if not _args.keep_cache:
            # the decode cache is ~660 MB per page (.npy 158 MB + .pkl 500 MB); keeping it for a
            # whole piece fills the disk, so drop it once the MusicXML is written.
            for _c in (pklPath, pklPath.replace(".pkl", ".npy"),
                       pklPath.replace(".pkl", "_staffList.pkl"),
                       pklPath.replace(".pkl", "_barlist.pkl")):
